 ...
 * @author Doug Lea
 */
public abstract class VkAbstractQueuedSynchronizer extends VkAbstractOwnableSynchronizer
	 ...
	 */
	protected VkAbstractQueuedSynchronizer() {
	}
	 ...
	 * @param update
	 *            新值
	 * @return 如果成功，则返回true；如果实际值与预期值不相等，则返回false（内存里面的实际值与预期值不相等，
	 *         意味着其他线程修改了内存中的值）
	 */
		...
		try {
			for (;;) {
				...
				if (shouldParkAfterFailedAcquire(p, node) && nanosTimeout > spinForTimeoutThreshold)
					VkLockSupport.parkNanos(this, nanosTimeout);
	...
	/**
	 * 试图在独占模式下获取对象状态。此方法应该查询是否允许它（即线程）在独占模式下获取对象状态，如果允许，则获取它。
	 * <p>
	 * 该方法总是由执行acquire方法的线程来调用。如果该方法报告失败，则 acquire
	 * 方法可以将线程加入队列（如果还没有将线程加入队列），直到获得其他某个线程释放了该线程的信号（相当于其他线程调用了notify()
	 * 或notifyAll()方法时，内置锁阻塞队列里面的线程将被唤醒）。<br/>
	 * 可以用此方法来实现 Lock.tryLock() 方法。
	 * </p>
	 * 默认实现将抛出 UnsupportedOperationException，因此子类必须重写该方法。
	 * 
	 * @param arg
	 *            acquire 参数。该值总是传递给 acquire
	 *            方法的那个值，或者是因某个条件等待而保存在条目上的值。该值是不间断的，并且可以表示任何内容。
	 *            <b>该方法总是由acquire方法调用，因此参数也是来自acquire方法。</b>
	 ...
	 */
	public class ConditionObject implements VkCondition, java.io.Serializable {
		...
		public final boolean awaitUntil(Date deadline) throws InterruptedException {
			...
			while (!isOnSyncQueue(node)) {
				...
				VkLockSupport.parkUntil(this, abstime);
				if ((interruptMode = checkInterruptWhileWaiting(node)) != 0)
					break;
			}
			if (acquireQueued(node, savedState) && interruptMode != THROW_IE)
		 ...
		 * @return {@code true} if owned
		 */
		final boolean isOwnedBy(VkAbstractQueuedSynchronizer sync) {
			return sync == VkAbstractQueuedSynchronizer.this;
		}
	...
	static {
		try {
			stateOffset = unsafe.objectFieldOffset(VkAbstractQueuedSynchronizer.class.getDeclaredField("state"));
			headOffset = unsafe.objectFieldOffset(VkAbstractQueuedSynchronizer.class.getDeclaredField("head"));
			tailOffset = unsafe.objectFieldOffset(VkAbstractQueuedSynchronizer.class.getDeclaredField("tail"));
