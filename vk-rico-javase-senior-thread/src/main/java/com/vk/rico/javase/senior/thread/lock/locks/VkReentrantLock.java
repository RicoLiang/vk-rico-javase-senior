...
public class VkReentrantLock implements Lock, java.io.Serializable {
	 ...
	 */
	public VkReentrantLock(boolean fair) {
	 ...
	 * 获取锁。
	 * <p>
	 * 如果该锁没有被另一个线程保持（即占有），则获取该锁并立即返回，将锁的保持计数设置为 1（即锁的占有计数器设置为1）。<br/>
	 * 如果当前线程已经保持该锁（即当前线程已经占有了该锁），则将保持计数加 1，并且该方法立即返回。<br/>
	 * 如果该锁被另一个线程保持，则出于线程调度的目的，禁用当前线程，并且在获得锁之前，该线程将一直处于休眠状态，此时锁保持计数被设置为 1。<br/>
	 * <b> 回顾锁：锁关联一个请求计数（acquisition count）和一个占有它的线程，锁重进入特性</b>
	 * </p>
	 * 
	 */
	public void lock() {
