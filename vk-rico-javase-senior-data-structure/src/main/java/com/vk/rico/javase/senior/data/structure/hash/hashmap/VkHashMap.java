package com.vk.rico.javase.senior.data.structure.hash.hashmap;
...
public class VkHashMap<K, V> extends VkAbstractMap<K, V> implements Map<K, V>, Cloneable, Serializable {
	 ...
	 * else 0.
	 */
	@SuppressWarnings({ "unchecked", "rawtypes" }) // for cast to Comparable
	 ...
	 *             nonpositive
	 */
	public VkHashMap(int initialCapacity, float loadFactor) {
		...
		// 处理initialCapacity参数，初始化threshold的值，threshold必须是2的n次幂，范围是2的0次幂到2的30次幂，在resize()方法中有用到
		// 当调用无参构造器时，threshold的值为0，因为没有对threshold进行初始化
	 ...
	 */
	public VkHashMap(int initialCapacity) {
	 ...
	 */
	public VkHashMap(Map<? extends K, ? extends V> m) {
	...
	final V putVal(int hash, K key, V value, boolean onlyIfAbsent, boolean evict) {
		...
		Node<K, V> p;
		int n, i;
		if ((tab = table) == null || (n = tab.length) == 0) { // 初始化table，并获取桶的数量
			// 初始化时走这里
			n = (tab = resize()).length;
		}
		if ((p = tab[i = (n - 1) & hash]) == null) { // 判断指定位置上的桶是否为NULL
			/**
			 * 1、(n - 1) & hash得到的下标值一定会在桶数量的范围内吗？<br/>
			 * 2、(n - 1)数组下标是从0开始的，如数组长度是8（即8个元素），那么最大下标是7。<br/>
			 * 3、如果这里返回false，即某个桶上已经有Node，就可以得出两个key的hash值相同（但这两个key不一定相等）。<br/>
			 * 4、当扩容之后，相同的hash，也可以得到相同位置的桶，当n=8、16、32...(2的n次幂)，<br/>
			 * (n - 1) & hash得到的下标值都是一样的，这也就是为什么Hash表的容量必须是2的n次幂(n>=0)，<br/>
			 * 扩容也是以倍数（容量*2）进行的原因。【这一点有待斟酌】<br/>
			 * 5、hash不同，n - 1相同，(n - 1) & hash可能得到相同的值（即相同的下标值），这就是hash碰撞
			 */
			tab[i] = newNode(hash, key, value, null);
		} else {
			/* hash碰撞（即hash冲突，两个key的hash值相同）的处理方式 */
			Node<K, V> e;
			K k;
			if (p.hash == hash && ((k = p.key) == key || (key != null && key.equals(k)))) {
				/* 两个key的hash值相同，并且相等，则将新的value更新旧的value */
				e = p;
		...
		++modCount;
		if (++size > threshold) {
			/* 被占用桶的数量超过阀值时，就会扩容，以增加桶的数量，扩容之后阀值也会加大 */
			resize();
		}
	...
	final Node<K, V>[] resize() {
		Node<K, V>[] oldTab = table; // 初始化时，table为null
		int oldCap = (oldTab == null) ? 0 : oldTab.length; // 初始化时，oldCap=0
		int oldThr = threshold; // 范围是2的0次幂到2的30次幂
		int newCap, newThr = 0;
		if (oldCap > 0) {
			if (oldCap >= MAXIMUM_CAPACITY) {
				...
				// newCap = oldCap << 1，容量翻倍；当容量达到16时，阀值也翻倍
				newThr = oldThr << 1; // double threshold
			}
		} else if (oldThr > 0) { // 初始化时，走这里，用初始threshold作为初始容量
			newCap = oldThr;
		} else {
			// 调用无参构造器时，threshold值为0，初始化时，就会使用默认的容量值和阀值
		...
		table = newTab;
		if (oldTab != null) {
			// 循环数组，将旧数据放到新的Hash表中（扩容之后的Hash表）
			for (int j = 0; j < oldCap; ++j) {
				Node<K, V> e;
				if ((e = oldTab[j]) != null) {
					oldTab[j] = null;
					if (e.next == null) {
						// e.hash & (newCap - 1)，计算元素下标值
						// 保证扩容之后，旧元素在Hash表中的位置不发生变化，这就是为什么Hash表中桶的数量必须是2的n次幂【有待斟酌】
						// 如初始容量是8，扩容了4被，第1个桶的下标是5，扩容之后，桶的新下标将是：4*5+1=21，后续的桶以该位置往后推1
						newTab[e.hash & (newCap - 1)] = e;
					} else if (e instanceof TreeNode) {
						((TreeNode<K, V>) e).split(this, newTab, j, oldCap);
					} else { // preserve order
						/* 处理hash碰撞的桶（以链表的形式保存着节点） */
	...
	final class KeySet extends AbstractSet<K> {
		...
		public final Spliterator<K> spliterator() {
			return new KeySpliterator<>(VkHashMap.this, 0, -1, 0, 0);
		}
	...
	final class Values extends AbstractCollection<V> {
		...
		public final Spliterator<V> spliterator() {
			return new ValueSpliterator<>(VkHashMap.this, 0, -1, 0, 0);
		}
	...
	final class EntrySet extends AbstractSet<Map.Entry<K, V>> {
		...
		public final Spliterator<Map.Entry<K, V>> spliterator() {
			return new EntrySpliterator<>(VkHashMap.this, 0, -1, 0, 0);
		}
	...
	@Override
	public Object clone() {
		VkHashMap<K, V> result;
		try {
			result = (VkHashMap<K, V>) super.clone();
	...
	static class HashMapSpliterator<K, V> {
		...
		HashMapSpliterator(VkHashMap<K, V> m, int origin, int fence, int est, int expectedModCount) {
			this.map = m;
			this.index = origin;
			this.fence = fence;
			this.est = est;
	...
	static final class KeySpliterator<K, V> extends HashMapSpliterator<K, V> implements Spliterator<K> {
		KeySpliterator(VkHashMap<K, V> m, int origin, int fence, int est, int expectedModCount) {
	...
	static final class ValueSpliterator<K, V> extends HashMapSpliterator<K, V> implements Spliterator<V> {
		ValueSpliterator(VkHashMap<K, V> m, int origin, int fence, int est, int expectedModCount) {
			super(m, origin, fence, est, expectedModCount);
		}
	...
	static final class EntrySpliterator<K, V> extends HashMapSpliterator<K, V> implements Spliterator<Map.Entry<K, V>> {
		EntrySpliterator(VkHashMap<K, V> m, int origin, int fence, int est, int expectedModCount) {
			super(m, origin, fence, est, expectedModCount);
		}
	 ...
	 */
	static final class TreeNode<K, V> extends VkLinkedHashMap.Entry<K, V> {
		 ...
		 * node.
		 */
		final Node<K, V> untreeify(VkHashMap<K, V> map) {
		 ...
		 */
		final TreeNode<K, V> putTreeVal(VkHashMap<K, V> map, Node<K, V>[] tab, int h, K k, V v) {
			Class<?> kc = null;
		 ...
		 * tree structure).
		 */
		final void removeTreeNode(VkHashMap<K, V> map, Node<K, V>[] tab, boolean movable) {
			int n;
		 ...
		 */
		final void split(VkHashMap<K, V> map, Node<K, V>[] tab, int index, int bit) {
			TreeNode<K, V> b = this;
