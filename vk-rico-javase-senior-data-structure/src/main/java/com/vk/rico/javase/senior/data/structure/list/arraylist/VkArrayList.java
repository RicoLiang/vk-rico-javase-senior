package com.vk.rico.javase.senior.data.structure.list.arraylist;
...
public class VkArrayList<E> extends AbstractList<E> implements List<E>, RandomAccess, Cloneable, java.io.Serializable {
	 ...
	 */
	public VkArrayList(int initialCapacity) {
	 ...
	 */
	public VkArrayList(Collection<? extends E> c) {
	...
	public Object clone() {
		try {
			VkArrayList<?> v = (VkArrayList<?>) super.clone();
	...
	private class Itr implements Iterator<E> {
		...
		public E next() {
			...
			Object[] elementData = VkArrayList.this.elementData;
		...
		public void remove() {
			...
			try {
				VkArrayList.this.remove(lastRet);
				cursor = lastRet;
				lastRet = -1;
		...
		public void forEachRemaining(Consumer<? super E> consumer) {
			...
			final int size = VkArrayList.this.size;
			int i = cursor;
			if (i >= size) {
				return;
			}
			final Object[] elementData = VkArrayList.this.elementData;
			if (i >= elementData.length) {
				throw new ConcurrentModificationException();
			}
	...
	private class ListItr extends Itr implements ListIterator<E> {
		...
		public void set(E e) {
			...
			try {
				VkArrayList.this.set(lastRet, e);
			} catch (IndexOutOfBoundsException ex) {
				throw new ConcurrentModificationException();
			}
		}
	...
	static final class ArrayListSpliterator<E> implements Spliterator<E> {
		...
		private final VkArrayList<E> list;
		private int index; // current index, modified on advance/split
		private int fence; // -1 until used; then one past last index
		private int expectedModCount; // initialized when fence set

		/** Create new spliterator covering the given range */
		ArrayListSpliterator(VkArrayList<E> list, int origin, int fence, int expectedModCount) {
